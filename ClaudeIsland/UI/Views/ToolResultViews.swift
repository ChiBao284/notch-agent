...
struct TodoWriteResultContent: View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            ForEach(Array(result.newTodos.enumerated()), id: \.offset) { _, todo in
                HStack(spacing: 6) {
                    ...
                    Text(todo.content)
                        .font(.system(size: 11))
                        .foregroundColor(.notchFG.opacity(todo.status == "completed" ? 0.4 : 0.7))
    ...
    private func todoColor(for status: String) -> Color {
        ...
        default: return .notchFG.opacity(0.4)
        }
    }
}
...
struct TaskResultContent: View {
    ...
    private var statusColor: Color {
        ...
        default: return .notchFG.opacity(0.5)
        }
    }
...
struct FileCodeView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            .padding(.vertical, 6)
            .background(Color.notchFG.opacity(0.06))
...
private enum DiffLineType {
    ...
    var textColor: Color {
        ...
        case .context: return .notchFG.opacity(0.5)
        }
    }
